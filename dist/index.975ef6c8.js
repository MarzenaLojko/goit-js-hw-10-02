...
countryListEl.previousElementSibling.addEventListener("input", debounce(onInput, DEBOUNCE_DELAY));
function onInput() {
    ...
    if (nameCountry === "" || nameCountry === " ") {
        ...
        return;
    } else (0, _fetchCountries.fetchCountries)(nameCountry).then((country)=>{
        if (country.length > 10) {
            ...
            countryListEl.nextElementSibling.innerHTML = "";
            countryListEl.innerHTML = country.map((i)=>{
                return `<div class="country-list_flag"><img src="${i.flags.svg}" alt="" width="32" /><div class="country-list_title">${i.name.common}</div></div>`;
            }).join("");
        } else {
            countryListEl.innerHTML = "";
            countryListEl.nextElementSibling.innerHTML = `<div class="country-info_flag"><img src="${country[0].flags.svg}" alt="" width="32" /><div class="country-info_title">${country[0].name.common}</div></div><div class="country-info_description"><b>Capital:</b> ${country[0].capital[0]}</div><div class="country-info_description"><b>Population:</b> ${country[0].population}</div><div class="country-info_description"><b>Languages:</b> ${Object.values(country[0].languages)[0]}</div>`;
        }
    }).catch((error)=>{
...
},{"./css/styles.css":"1CY4s","notiflix/build/notiflix-notify-aio":"eXQLZ","./fetchCountries":"fTQqv","lodash.debounce":"3JP5n"}],"1CY4s":[function() {},{}],"eXQLZ":[function(require,module,exports) {
var global = arguments[3];
/*
...
const fetchCountries = (name)=>fetch(`https://restcountries.com/v3.1/name/${name}?fields=name,capital,population,flags,languages`).then((response)=>{
